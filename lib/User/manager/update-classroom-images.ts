...
export async function addClassroomImages(instituteId: string, formData: FormData) {
    try {
        ...
        await prisma.institute.update({
            ...
            data: { classroomImages: { push: secureUrl } }
        });
...
export async function removeClassroomImage(instituteId: string, imageUrlToRemove: string) {
    try {
        const institute = await prisma.institute.findUnique({ where: { id: instituteId } });
        if (!institute) return { success: false, error: "Not found." };

        const updatedImages = institute.classroomImages.filter(url => url !== imageUrlToRemove);

        await prisma.institute.update({
            where: { id: instituteId },
            data: { classroomImages: updatedImages }
        });
