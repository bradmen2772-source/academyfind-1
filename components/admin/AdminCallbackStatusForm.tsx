...
import { updateCallbackStatusAndNote } from "@/app/(af-ass-manage)/af-ass-manage/instituteCallbacks/actions";

export default function AdminCallbackStatusForm({ 
    callbackId, 
    initialStatus, 
    initialNotes 
}: { 
    ...
    const [status, setStatus] = useState(initialStatus === "NEW" ? "PENDING" : initialStatus);
    const [notes, setNotes] = useState(initialNotes || "");

    const handleSave = async () => {
        setIsPending(true);
        const res = await updateCallbackStatusAndNote(callbackId, status, notes);
        if (res.success) {
        ...
        <div className="space-y-4">
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {statusOptions.map((opt) => {
                        const isSelected = status === opt.value || (opt.value === "PENDING" && status === "NEW");
                        return (
                            <button
                                key={opt.value}
                                type="button"
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Add internal notes about this callback here..."
            ...
            <div className="flex justify-end pt-2">
                <Button 
                    disabled={isPending}
                    onClick={handleSave}
                    className="rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white px-5 font-bold text-xs gap-1.5 shadow-sm cursor-pointer"
                >
